...
aws iam put-role-policy --role-name "$ROLE" \
  ...
  --policy-document '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Action":[
    ...
    "bedrock:ListInferenceProfiles","bedrock:GetInferenceProfile","bedrock:ListTagsForResource",
...
| ≤ 1.11.3 | ⚠️ 判定口径变更(1.12.0):**删除 IAM principal 存在性豁免**,合规=打标正确的 inference profile(runtime)/project(mantle),其余一律告警。升级后告警可能变多(修复漏报) |
| ≤ 1.12.0 | 分账告警窗口若曾在页面调得比检查频率小,旧版存在**检查盲区漏报**;1.12.1 起自动把窗口抬齐到检查间隔(日志可见提示),无需操作 |
| ≤ 1.13.0 | ⚠️ 标签值核查口径统一(1.13.1):此前 mantle **project 标签只查非空、不比对值**,设置期望值后打错值的 project 会被当合规漏过;升级后与 profile/principal 一致按值比对,打错值的 project 用量会开始出现在告警中(修复漏报)。「map-migrated 标签值」输入框已移至 **⚙️ 配置页顶部**(1.13.0),原「真实账单」面板内不再显示 |
| ≤ 1.8.0 | 1.9.0 曾新增 mantle 专项检查(独立 15 分钟 rule),**1.11.0 起已并入主告警**,直升后不存在独立专项,`MANTLE_RATE` 参数已移除 |
...
6. ⚙️ 配置页顶部可见「🏷️ map-migrated 标签值」面板,已设期望值的确认值还在(存于 S3,升级不丢)
7. (设置了期望标签值的)找一个标签值不符的 project/profile,确认用量表分项标「⚠️ 标签无效」而非合规
